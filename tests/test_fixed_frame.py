"""Run main.c's fixed-size dialog frame against stubbed window calls.
...
Run: python3 tests/test_fixed_frame.py
"""
...
FRAME = 'g_webviewFrameSize'
DIALOG = 'g_webviewHwnd'
PROC = 'WebViewWndProc'
CLASS = 'APIMonitorWebViewWnd'
...
class FixedFrameTests(unittest.TestCase):
    def test_frame_refuses_user_sizing(self):
        ...
        with tempfile.TemporaryDirectory(prefix='apimonitor-frame-') as directory:
            path = Path(directory)
    ...
    def test_every_dialog_size_goes_through_the_frame(self):
        ...
        self.assertIn('FixedFrameInit(' + DIALOG + ', &' + FRAME + ');',
                      SOURCE[creation.end():creation.end() + 200])
